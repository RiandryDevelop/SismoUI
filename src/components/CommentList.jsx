...
const updateComment = (updatedComment) => {
  ...
  setComments(updatedComments);
      ...
      const onEdit = async (comment_id , commentBody) => {
        ...
        if (updatedCommentsSaved.id && updatedCommentsSaved.body) {
          setCommentExist(true);
          try {
            const response = await fetch(`${API}comments/${updatedCommentsSaved.id}`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
                'Cache-Control': 'no-cache'
              },
              body: JSON.stringify({ body: updatedCommentsSaved.body }),
            });
      
            if (response.ok) {
              const updatedComments = comments.map(comment => {
                if (comment.id === updatedCommentsSaved.id) {
                  return { ...comment, body: updatedCommentsSaved.body };
                }
                return comment;
              });
      ...
      <div key={index} className="flex items-start mb-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center space-x-2">
            <button onClick={() => onEdit(comment.id,comment.body)} className="text-xs text-gray-600 hover:text-gray-900 focus:outline-none">
              Editar
            </button>
